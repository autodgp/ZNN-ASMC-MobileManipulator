...
For comparative experiments, the corresponding results are generated by running the provided simulation scripts with different activation-function (AF) settings and controller configurations (e.g., by modifying the AF-related functions and `ctrl.py`). The repository provides the main implementations required for reproduction, while the comparative data should be obtained by executing the simulations under the desired settings.
